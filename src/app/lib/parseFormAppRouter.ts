...
export async function nextRequestToIncomingMessage(req: NextRequest): Promise<IncomingMessage> {
...
export async function parseFormAppRouter(
  ...
  const form = formidable({
    multiples: true,
    uploadDir: process.env.NODE_ENV === 'development' ? 'public/uploads' : '/tmp',
    keepExtensions: true,
  });
